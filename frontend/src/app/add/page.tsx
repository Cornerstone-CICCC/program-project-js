...
export default function AddIngredientPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !expiryDate) {
      toast.error("Please enter the ingredient name and expiration date! 🍎");
      return;
    }

    setLoading(true);

    const savingToast = toast.loading("Saving your ingredient...");
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`${name} has been added! ✨`, { id: savingToast });
        router.refresh();
        setTimeout(() => {
          router.push("/");
        }, 100);
